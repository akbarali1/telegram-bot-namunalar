...
function botpostid($input) {
    ...
    if (!empty($user_text == "guruh")) {
        sendTelegram('sendMessage', array('chat_id' => $chat_id, 'text' => json_encode($data)));
...
#jpgtopdf'));
            ...
            unlink($chat_id . '.pdf');
            $count=$pdo->prepare("DELETE FROM jpgtopdf WHERE tgid=:tgid");
            $count->bindParam(":tgid", $chat_id ,PDO::PARAM_INT);
            $count->execute();
            exit();
        }
    } else {
